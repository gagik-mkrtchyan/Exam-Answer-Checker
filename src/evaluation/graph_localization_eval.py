...
def evaluate_graph_localization_model(model_path, data_path, 
                                     imgsz=640, 
                                     conf=0.001, 
                                     iou=0.6,
                                     device=None,
                                     ...
                                     verbose=True):
    """
    Evaluate trained YOLOv8 model for graph localization task
    ...
    # Evaluate the model
    print(f"Evaluating graph localization model: {model_path}")
    results = model.val(
        data=data_path,
        imgsz=imgsz,
        conf=conf,
    ...
    if verbose:
        print("\n=== Graph Localization Model Evaluation Results ===")
...
def evaluate_on_test_images(model_path, test_images_dir, 
    ...
    results = model.predict(
        ...
        show_conf=show_conf,
        project=output_dir,
        name="graph_localization_inference",
        exist_ok=True
    )
    
    print(f"Inference completed. Results saved to: {output_dir}/graph_localization_inference")
    return results

def main():
    """Main function for command line usage"""
    parser = argparse.ArgumentParser(description="Evaluate Graph Localization Model")
    parser.add_argument("--model", type=str, required=True, 
                       help="Path to trained model (.pt file)")
    parser.add_argument("--data", type=str, 
                       default="graph localization.v2i.yolov8/data.yaml",
    ...
    if args.task in ["val", "both"]:
        ...
        evaluate_graph_localization_model(
...
if __name__ == "__main__":
    ...
    model_path = "runs/detect/train111/weights/best.pt"  # Path to your trained model
    
    # Evaluate on validation set
    results = evaluate_graph_localization_model(
        model_path=model_path,
        data_path="graph localization.v2i.yolov8/data.yaml",
        verbose=True
    )
